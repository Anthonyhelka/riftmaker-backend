...
const createSpectatorFile = async (gameId, observerKey) => {
  const content = `
    @REM Spectate by Riftmaker
    SETLOCAL enableextensions enabledelayedexpansion 
    @echo off
    
    echo -----------------------
    echo Spectate by Riftmaker
    echo -----------------------
    ...
    if exist "%RADS_PATH%\\Game" (
      ...
      if exist "League of Legends.exe" (                                                
        @start "" "League of Legends.exe" "spectator spectator.na2.lol.riotgames.com:80 ${observerKey} ${gameId} NA1" "-UseRads" "-Locale=!locale!" "-GameBaseDir=.."
        goto :eof
      )
    )
